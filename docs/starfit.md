...
$$\text{NOR}_{up} = \min \left( \max \left( A + B \cdot \sin \, 2 \pi \omega t + C \cdot \cos \, 2 \pi \omega t, \; \hat{S}_{min} \right), \; \hat{S}_{max} \right)$$

$$\text{NOR}_{\text{up}} = \min \left( \max \left( A + B \cdot \sin \, 2 \pi \omega t + C \cdot \cos \, 2 \pi \omega t, \; \hat{S}_{\text{min}} \right), \; \hat{S}_{\text{max}} \right)$$

$$\text{NOR}_{low} = \min \left( \max \left( a + b \cdot \sin \, 2 \pi \omega t + c \cdot \cos \, 2 \pi \omega t, \; \hat{s}_{min} \right), \; \hat{s}_{max} \right)$$
...
In several reservoirs, the minimum release ($R_{min}$) is above the minimum of the harmonic release ($\tilde{R}$). This means that, at some points of the year, the release within NOR is lower than the release below NOR, which is nonesense.

I have added a new constraint to the release within NOR, to be at least equal to the minimum release:

$$R_{NOR} = \max \left( \min \left( \bar{I} \cdot \left( \tilde{R}_t + \epsilon_t \right) + \bar{I}, \, R_{max} \right) , \, R_{min} \right)$$
