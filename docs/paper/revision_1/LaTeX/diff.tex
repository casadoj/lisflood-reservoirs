...
%DIF DEL ./submission/LaTeX/manuscript.tex   Tue Jun  9 14:06:20 2026
%DIF ADD ./revision/LaTeX/manuscript.tex     Mon Jun 15 16:01:22 2026
%% ---------------------------------
...
\newcommand{\calGWW}{OBJ-V-GWW} %DIF > 
\newcommand{\KGEmod}{$\text{KGE}'$} %DIF > 
...
A novel aspect of this study is the exploration of a decoupled calibration strategy\DIFaddbegin \DIFadd{, where the reservoir routines are evaluated in isolation from the rest of the catchment hydrology}\DIFaddend . Rather than calibrating reservoir parameters simultaneously with the rest of the hydrological model against downstream river discharge—a process often plagued by equifinality \citep{Beven2001}—we \DIFdelbegin \DIFdel{define reservoir parameters in advance using specific reservoir records. These pre-determined parameters are then integrated into the hydrological model for the subsequent calibration of the remaining parameters}\DIFdelend \DIFaddbegin \DIFadd{model reservoir management as a standalone system. To achieve this, we selected reservoirs with observed inflow, outflow, and storage records}\DIFaddend . This approach ensures that reservoir parameters are sensitive to actual \DIFdelbegin \DIFdel{reservoir dynamics rather than }\DIFdelend \DIFaddbegin \DIFadd{operations rather than compensating for upstream hydrological biases or }\DIFaddend being masked by gauge observations far downstream.
...
%DIF <  Missing:
%DIF <  * The regionalization approach
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Data}
\label{sec:data}
...
\begin{enumerate}
    ...
    \item Hydrological Impact: \DIFdelbegin \DIFdel{Following \mbox{%DIFAUXCMD
\citet{Shrestha2024}}\hskip0pt%DIFAUXCMD
, we excluded }\DIFdelend \DIFaddbegin \DIFadd{Large-scale hydrological models such as GloFAS or EFAS are primarily focused on modelling river streamflow. For the sake of parsimony, we only want to include in those systems }\DIFaddend "\DIFdelbegin \DIFdel{non-disruptive}\DIFdelend \DIFaddbegin \DIFadd{disruptive}\DIFaddend " reservoirs—those that \DIFdelbegin \DIFdel{do not }\DIFdelend significantly alter the \DIFdelbegin \DIFdel{natural }\DIFdelend \DIFaddbegin \DIFadd{downstream }\DIFaddend flow regime. \DIFdelbegin \DIFdel{This was quantified }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Shrestha2024} }\hskip0pt%DIFAUXCMD
devised two thresholds that identify disruptive reservoirs }\DIFaddend using the Degree of Regulation ($\text{DOR}$; Eq.~\ref{eq:degree_of_regulation}) and the Degree of Disruptivity ($\text{DOD}$; Eq.~\ref{eq:degree_of_disruptivity}). \DIFdelbegin \DIFdel{Reservoirs with $\text{DOR} < 0.08$ or $\text{DOD} < 0.06$}\DIFdelend \DIFaddbegin \DIFadd{Following their recommendations, only reservoirs with $\text{DOR} \geq 0.08$ and $\text{DOD} \geq 0.06$}\DIFaddend ~m were \DIFdelbegin \DIFdel{discarded}\DIFdelend \DIFaddbegin \DIFadd{selected}\DIFaddend :
    \begin{subequations}
...
\begin{figure*}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\textwidth]{figures/fig01.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/fig01.pdf}
    \DIFaddendFL \caption{Distribution of the 164 reservoirs from the ResOpsUS dataset used in the benchmarking. Colors represent the \DIFdelbeginFL \DIFdelFL{catchment area }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{main reservoir use }\DIFaddendFL and point size the \DIFdelbeginFL \DIFdelFL{storage capacity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{degree of regulation}\DIFaddendFL .}
...
%DIF < Furthermore, the original routine exhibits a sharp discontinuity when switching between normal (quadratic) and flood (linear) modes, often producing artificial spikes in the release time series when $I_t$ exceeds $Q_f$. To address this, we implemented a sigmoid weighting function (Eq.~\ref{eq:sigmoid_weighing}) to ensure a smooth transition between the two modes:
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \begin{equation}
%DIF < \label{eq:combined_release} 
%DIF <     Q_{t} = (1 - w) \cdot Q_{normal} + w \cdot Q_{flood} 
%DIF < \end{equation} 
%DIF < \begin{equation}
%DIF < \label{eq:sigmoid_weighing} 
%DIF <     w = \frac{1}{1 + e^{-(I_t - \mu)\cdot \sigma}} 
%DIF < \end{equation}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < where $w$ is the weight, and $\mu$ and $\sigma$ are the location and scale parameters of the transformed inflow. This weighted release allows for a gradual increase in discharge as the inflow approaches the flood threshold.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend While \citet{Hanazaki2022} utilized default parameters, we calibrated five parameters (Table~\ref{tab:parameters_camaflood}) to ensure the routine has comparable degrees of freedom to the other routines. To maintain consistency with the LISFLOOD benchmark, three parameters ($\alpha$, $\delta$, $\epsilon$) share identical definition across both models.
...
where $t$ is the time (day or week) of the year. The NOR has 10 parameters: six defining the harmonic curves and four capping the maximum and minimum filling \DIFaddbegin \DIFadd{(Table~\ref{tab:parameters_starfit})}\DIFaddend . To fit these curves, the model uses only the three most extreme (highest and lowest) observed storage for each calendar week (Fig.~\ref{fig:starfit}), ensuring the NOR captures the envelope of historical operations.
...
We modified the definitions of the outflow for both below and above the NOR, as the original definitions produced in some cases noisy releases. In both cases, we have introduced a linear interpolation between a fixed \DIFdelbegin \DIFdel{minium }\DIFdelend \DIFaddbegin \DIFadd{minimum }\DIFaddend or maximum outflow ($Q_{\min}$ or $Q_{\max}$) and the outflow associated  with the respective NOR boundary ($Q_{\text{NOR}_{\text{low}},t}$ or $Q_{\text{NOR}_{\text{up}},t}$). Within the NOR, the standardized outflow ($\hat{Q}_{t}$) is modeled as the sum of an harmonic component ($\hat{Q}_{harm,t}$) and a linear component  ($\hat{Q}_{\text{lin},t}$) that accounts for the current storage and inflow:
...
The complete release model comprises 7 parameters \DIFaddbegin \DIFadd{(Table~\ref{tab:parameters_starfit})}\DIFaddend , fitted using the full historical record of weekly releases (Fig.~\ref{fig:starfit}). Additionally, the minimum and maximum releases ($Q_{\min}$ and $Q_{\max}$) are estimated as quantiles of the observed daily inflows; we have changed the original values of these quantiles to 1\% and 99\%, respectively.

\DIFaddbegin \begin{table*}[t]
    \centering
    \begin{threeparttable}
        \caption{\DIFaddFL{Parameters in the STARFIT reservoir module.}}
        \label{tab:parameters_starfit}
        \begin{tabularx}{\textwidth}{llXl}
            \toprule
            \textbf{\DIFaddFL{Variable}} & \textbf{\DIFaddFL{Parameter}} & \textbf{\DIFaddFL{Description}} & \textbf{\DIFaddFL{Definition}} \\
            \midrule
            \textbf{\DIFaddFL{Storage}} & \DIFaddFL{$A, a$}\tnote{*} & \DIFaddFL{Average filling }& \DIFaddFL{Eq.~\ref{eq:starfit_nor} }\\
                             & \DIFaddFL{$B, b$}\tnote{*} & \DIFaddFL{Coefficient of the sine component }& \DIFaddFL{Eq.~\ref{eq:starfit_nor} }\\
                             & \DIFaddFL{$C, c$}\tnote{*} & \DIFaddFL{Coefficient of the cosine component }& \DIFaddFL{Eq.~\ref{eq:starfit_nor} }\\
                             & \DIFaddFL{$\hat{V}_{\min}, \hat{v}_{\min}$}\tnote{*} & \DIFaddFL{Lower limit to the filling }& \DIFaddFL{Eq.~\ref{eq:starfit_nor} }\\
                             & \DIFaddFL{$\hat{V}_{\max}, \hat{v}_{\max}$}\tnote{*} & \DIFaddFL{Upper limit to the filling }& \DIFaddFL{Eq.~\ref{eq:starfit_nor} }\\
            \midrule
            \textbf{\DIFaddFL{Release}} & \DIFaddFL{$Q_{\min}$ }& \DIFaddFL{Minimum release }& \DIFaddFL{$Q_1$}\tnote{**} \\
                             & \DIFaddFL{$Q_{\max}$ }& \DIFaddFL{Maximum release }& \DIFaddFL{$Q_{99}$}\tnote{**} \\
                             & \DIFaddFL{$d$ }& \DIFaddFL{Coefficient of the sine component in the first release harmonic }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$e$ }& \DIFaddFL{Coefficient of the cosine component in the first release harmonic }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$f$ }& \DIFaddFL{Coefficient of the sine component in the second release harmonic }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$g$ }& \DIFaddFL{Coefficient of the cosine component in the second release harmonic }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$h$ }& \DIFaddFL{Intercept in the linear release }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$i$ }& \DIFaddFL{Coefficient of the linear release related to the current storage }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
                             & \DIFaddFL{$j$ }& \DIFaddFL{Coefficient of the linear release related to the current inflow }& \DIFaddFL{Eq.~\ref{eq:starfit_release_linear} }\\
             \bottomrule
        \end{tabularx}
        \begin{tablenotes}
            \small
            \item[\DIFaddFL{*}] \DIFaddFL{Upper case refers to the upper NOR, and lower case to the lower NOR.
            }\item[\DIFaddFL{**}] \DIFaddFL{Percentiles of the observed daily flows.
        }\end{tablenotes}
    \end{threeparttable}
\end{table*}

\DIFaddend \subsection{Experimental design}
...
Performance was quantified using the modified Kling-Gupta Efficiency (\DIFdelbegin \DIFdel{$\text{KGE}'$}\DIFdelend \DIFaddbegin \DIFadd{\KGEmod}\DIFaddend ) \citep{kling2012}, which decomposes performance into correlation, bias, and variability components. For the bivariate experiment, we integrated the individual \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend scores into a single multi-objective metric:

{\small
\begin{subequations}
    \label{eq:metrics}
    \begin{align}
        &\text{KGE}' = 1 - \DIFdelbegin \DIFdel{\sqrt{(r - 1)^2 + \Bigl(\frac{\mu_{\text{sim}}}{\mu_{\text{obs}}} - 1\Bigr)^2 + \Bigl(\frac{\text{CV}_{\text{sim}}}{\text{CV}_{\text{obs}}} - 1\Bigr)^2} }\DIFdelend \DIFaddbegin \DIFadd{\sqrt{(\alpha - 1)^2+ (\beta - 1)^2 + (\rho - 1)^2 } }\DIFaddend \label{eq:kge} \\
...
%DIFDELCMD < %%%
\DIFdel{where $r$ }\DIFdelend \DIFaddbegin \DIFadd{where $\rho$ }\DIFaddend is the Pearson correlation coefficient, \DIFdelbegin \DIFdel{$\mu$ is the mean, and $\text{CV}$ is the coefficient of variation.
}\DIFdelend \DIFaddbegin \DIFadd{$\alpha$ represents variability (ratio of coefficients of variation), and $\beta$ is the bias (ratio of means), defined as:
}{\small
\begin{align}
    &\DIFadd{\beta = \frac{\mu_{\text{sim}}}{\mu_{\text{obs}}} \label{eq:kge_beta} }\\
    &\DIFadd{\alpha = \frac{\text{CV}_{\text{sim}}}{\text{CV}_{\text{obs}}} \label{eq:kge_alpha}
}\end{align}
}
\DIFaddend 
...
%DIF < \subsection{Regionalization}
%DIF < \label{sec:regionalization}
\DIFdelbegin %DIFDELCMD < 
...
\begin{figure*}
    ...
    \caption{Comparison of the model performance depending on the variable(s) targeted in  the calibration. The panels \DIFdelbeginFL \DIFdelFL{corresponds }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correspond }\DIFaddendFL to the three reservoirs routines that were calibrated: a) LISFLOOD, b) CaMa-Flood and c) mHM. Within each panel, three groups of box plots show the performance in terms of outflow, storage, and both variables together. Four parameter sets are compared —default \DIFdelbeginFL \DIFdelFL{values}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{parameters}\DIFaddendFL , calibration of outflow (\calQ), storage (\calV) and both variables (\calQV)— indicated by colors. The dots represent individual reservoirs, indicating the distribution of the overall performance.}
    \label{fig:compare_variables}
\end{figure*}

The default parameterization (gray) provides a reasonably effective baseline for reservoir outflow across all three routines, with median \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend values of 0.44, 0.52, and 0.60 for LISFLOOD, CaMa-Flood, and mHM, respectively. However, these default parameters fail to capture storage dynamics effectively; median \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend scores for storage drop to 0.30, 0.36, and 0.29, respectively. The significantly larger interquartile range (IQR) in these scores indicates the high variability in storage performance across the 164 reservoirs. 

As expected, each routine achieves its peak performance for a specific variable when that variable is the sole target of the calibration. However, there is a clear trade-off when we look at how the models perform on variables they weren't trained on. Calibrating outflow (\calQ) severely degrades storage performance; in LISFLOOD and CaMa-Flood, this configuration yields the lowest median \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend for storage among all experiments; in mHM, while the median is slightly higher than the default parameters, the dispersion between reservoirs is remarkably large. Conversely, calibrating to storage does not result in a similar degradation of outflow performance. In all three routines, \calV\ maintains outflow \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend values that are superior to the default parameterization.
...
\begin{figure*}
    ...
    \caption{Comparison of the model performance depending on the reservoir routine. The panels \DIFdelbeginFL \DIFdelFL{corresponds }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correspond }\DIFaddendFL to three of the experiments: a) default \DIFdelbeginFL \DIFdelFL{values}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{parameters}\DIFaddendFL , b) calibration of storage (\calV), c) bivariate calibration of outflow and storage (\calQV). Within each panel, three groups of box plots show the performance in terms of outflow, storage, and both variables together. Four reservoir routines are compared— LISFLOOD, CaMa-Flood, mHM and STARFIT— indicated by colors. The dots represent individual reservoirs, indicating the distribution of overall performance.}
...
The inverted half-pyramid assumption proved to be a robust approximation for the majority of the study sites. The median \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend for reservoir level estimation is 0.72, with 75\% of the reservoirs achieving a \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend at least 0.44. From the three components of \DIFdelbegin \DIFdel{$\text{KGE}'$}\DIFdelend \DIFaddbegin \DIFadd{\KGEmod}\DIFaddend , the variability is the main cause of poor perfomance (median value of 1.25), whereas bias and correlation showed an overall good performance (median values of 1.00 in both cases). The accuracy of the level calculation is highly sensitive to the quality of the static attributes, specifically the crest elevation ($Z_{\max}$) and dam height ($H_{\text{dam}}$) (Eq.~\ref{eq:level}). As discussed in Section~\ref{sec:reservoir_attributes}, errors in global databases such as GRanD or GDW can propagate into these elevation estimates; therefore, data cleaning and the use of locally validated attributes are essential for accurate water-level modeling.
...
A similar constraint applies to STARFIT, although its performance was not markedly superior to the more parsimonious CaMa-Flood routine. A distinct advantage of STARFIT for global applications is its flexibility across different temporal resolutions. \DIFaddbegin \DIFadd{The harmonic functions can be fitted at coarse or irregular temporal resolutions (such as those in satellite products) and be executed at daily resolution by adjusting the frequency term ($\omega$). }\DIFaddend For instance, \citet{Steyaert2025} recently demonstrated a global application of the STARFIT routine within the PCR-GLOBWB2 hydrological model by leveraging satellite-derived storage from the GloLAKES dataset \citep{Hou2024} to define the storage normal operating range (NOR). \DIFaddbegin \DIFadd{To explore this potential, Appendix~\ref{app:gww} provides a comparative analysis of the four reservoir routines when calibrated using satellite-derived storage estimates.
}\DIFaddend 
...
\begin{figure*}
    ...
    \DIFdelbeginFL %DIFDELCMD < \label{fig:gww_performance}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:gww_storage_performance}
\DIFaddendFL \end{figure*}

Despite the limited sample size, GWW storage estimates demonstrate relatively high skill, with a median \DIFdelbegin \DIFdel{$\text{KGE}'$ }\DIFdelend \DIFaddbegin \DIFadd{\KGEmod\ }\DIFaddend of 0.65 and 75\% of the reservoirs exceeding a score of 0.51. In terms of decomposition, GWW tends to overestimate total storage (median bias of 1.12 with an IQR of 0.13) while slightly underestimating variability (median of 0.91, IQR 0.37). Notably, the temporal dynamics are captured accurately, reflected by a median correlation of 0.88 (IQR 0.10).

\DIFaddbegin \DIFadd{To evaluate the feasibility of utilizing satellite products in future applications, we conducted a fifth experiment (\calGWW) wherein the reservoir schemes were calibrated against the storage estimates from GWW. This setup mirrors the univariate storage calibration experiment (\calV), with the distinct modification that the optimization target is a satellite-derived product rather than in situ observations. Because the STARFIT formulation requires both storage and outflow time series for its fitting procedure, this specific model was parameterized using observed outflow alongside the satellite-estimated storage. Figure~\ref{fig:gww_model_performance} illustrates the performance of this experiment validated against in situ observations (i.e., evaluated against the actual reservoir storage records rather than the satellite data used during optimization).
}

\begin{figure*}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/figA2.pdf}
    \caption{\DIFaddFL{Comparison of model performance across different reservoir routines. The panels represent two experiments: a) default reservoir parameters, and b) univariate calibration using reservoir storage estimated by the Global Water Watch (\calGWW). Within each panel, three groups of box plots show the performance in terms of outflow, storage, and both variables combined. Four reservoir routines are compared— LISFLOOD, CaMa-Flood, mHM and STARFIT— indicated by colors. The dots represent individual reservoirs, indicating the distribution of overall performance.}}
    \label{fig:gww_model_performance}
\end{figure*}

\DIFadd{Calibration using the satellite product consistently yields performance improvements relative to the default parameter baselines. The LISFLOOD and CaMa-Flood routines appear to be particularly sensitive to the constraints of the satellite data. Both models experience a substantial surge in storage simulation performance, with median \KGEmod\ values increasing by 0.57 and 0.48 for LISFLOOD and CaMa-Flood, respectively. Interestingly, this enhancement does not impact outflow performance in LISFLOOD, whereas it noticeably degrades it in CaMa-Flood (a loss of 0.18 in median \KGEmod). 
}

\DIFadd{Conversely, the satellite-based calibration yields negligible improvements for the mHM routine (gains of only 0.07 and 0.09 in median \KGEmod\ for storage and outflow, respectively), resulting in poor overall performance. This outcome contrasts sharply with the original \calV\ experiment, where univariate in situ storage calibration yielded good performance across all routines, including mHM.
}

\DIFadd{While STARFIT emerges as the highest-performing routine in the \calGWW\ experiment, this comparison is inherently uneven. Because the fitting of this routine required a combination of satellite-estimated storage and observed in situ outflow, its setup is actually a bivariate calibration.
}

\DIFadd{These findings must be interpreted with caution. The evaluation is constrained by a small sample size ($n = 12$) that features reservoirs where the default parameters perform poorly.
}

\section{\DIFadd{Analysis of the \KGEmod\ components}}
\label{app:kge_components}

\DIFadd{Figure~\ref{fig:kge_components} shows the performance of the multiple simulation runs disaggregated into the individual \KGEmod\ components: variability ($\alpha$), bias ($\beta$), and correlation ($\rho$).
}\begin{figure*}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/figB1.pdf}
    \caption{\DIFaddFL{Comparison of the model performance in terms of variability ($\alpha$), bias ($\beta$) and correlation ($\rho$). The columns correspond to the four experiments: default parameters, calibration of outflow (\calQ), calibration of storage (\calV), and bivariate calibration of outflow and storage (\calQV). Within each panel, two groups of box plots show the performance in terms of outflow and storage. Four reservoir routines are compared— LISFLOOD, CaMa-Flood, mHM and STARFIT— indicated by colors. The dots represent individual reservoirs, indicating the distribution of overall performance.}}
    \label{fig:kge_components}
\end{figure*}

\DIFadd{The univariate calibration of outflow (\calQ) significantly degrades both the variability and bias of simulated storage. Conversely, the univariate calibration of storage (\calV) only marginally affects the correlation of simulated outflow and has a negligible impact on outflow variability and bias.
}

\DIFadd{The superior performance of the mHM routine in the bivariate calibration (\calQV) is driven by its more accurate representation of outflow variability and correlation, as well as storage variability and bias.
}

\section{\DIFadd{Performance by reservoir main use}}
\label{app:performance_use}

\DIFadd{The reservoir routines evaluated in this study are applied uniformly, regardless of the primary purpose of the reservoir. Table~\ref{tab:kge_use} analyses how the performance of these routines varies depending on the main reservoir use (as classified in the GRanD database). We exclusively present the values for the bivariate calibration, as it has been shown to yield the highest overall performance and provides the only equitable benchmark for the STARFIT model.
}
...
\begin{table*}[t]
    ...
    \begin{tabular}{l rrrr @{\hspace{0.7cm}} rrrr @{\hspace{0.7cm}} r}
        \toprule
         \textbf{\DIFaddFL{Main use}}  & \multicolumn{4}{c}{\textbf{Storage}} & \multicolumn{4}{c}{\textbf{Outflow}} & \textbf{\DIFaddFL{Count}}\\
          & \DIFaddFL{LISFLOOD }& \DIFaddFL{CaMa-Flood }& \DIFaddFL{mHM }& \DIFaddFL{STARFIT }& \DIFaddFL{LISFLOOD }& \DIFaddFL{CaMa-Flood }& \DIFaddFL{mHM }& \DIFaddFL{STARFIT }& \\
         \midrule
        \DIFaddFL{Flood control }& \DIFaddFL{0.63 }& \textbf{\DIFaddFL{0.74}} & \DIFaddFL{0.70 }& \DIFaddFL{0.73 }& \DIFaddFL{0.57 }& \DIFaddFL{0.50 }& \textbf{\DIFaddFL{0.56}} & \DIFaddFL{0.46 }& \DIFaddFL{66 }\\
        \DIFaddFL{Hydropower    }& \DIFaddFL{0.72 }& \DIFaddFL{0.75 }& \textbf{\DIFaddFL{0.77}} & \DIFaddFL{0.69 }& \DIFaddFL{0.69 }& \DIFaddFL{0.66 }& \textbf{\DIFaddFL{0.72}} & \DIFaddFL{0.68 }& \DIFaddFL{22 }\\
        \DIFaddFL{Irrigation    }& \DIFaddFL{0.65 }& \DIFaddFL{0.67 }& \textbf{\DIFaddFL{0.82}} & \DIFaddFL{0.73 }& \DIFaddFL{0.64 }& \DIFaddFL{0.60 }& \textbf{\DIFaddFL{0.79}} & \DIFaddFL{0.76 }& \DIFaddFL{54 }\\
        \DIFaddFL{Navigation    }& \DIFaddFL{0.24 }& \DIFaddFL{0.28 }& \textbf{\DIFaddFL{0.35}} & \DIFaddFL{0.33 }& \textbf{\DIFaddFL{0.87}} & \DIFaddFL{0.86 }& \DIFaddFL{0.86 }& \DIFaddFL{0.84 }& \DIFaddFL{4  }\\
        \DIFaddFL{Recreation    }& \DIFaddFL{0.00 }& \DIFaddFL{0.10 }& \DIFaddFL{-0.34 }& \textbf{\DIFaddFL{0.48}} & \DIFaddFL{0.62 }& \DIFaddFL{0.60 }& \textbf{\DIFaddFL{0.66}} & \DIFaddFL{0.55 }& \DIFaddFL{1  }\\
        \DIFaddFL{Water supply  }& \DIFaddFL{0.55 }& \textbf{\DIFaddFL{0.74}} & \DIFaddFL{0.73 }& \DIFaddFL{0.56 }& \DIFaddFL{0.67 }& \DIFaddFL{0.65 }& \textbf{\DIFaddFL{0.71}} & \DIFaddFL{0.60 }& \DIFaddFL{17 }\\
        \bottomrule
    \end{tabular}
    \caption{\DIFaddFL{Model performance (in terms of median \KGEmod) in the bivariate calibration disaggregated by main reservoir use and reservoir routine. The table shows performance for both target variables: storage and outflow. Bold values indicate the best-performing routine for each reservoir use and target variable, and the }\textit{\DIFaddFL{Count}} \DIFaddFL{column indicates the number of reservoirs available in the dataset for that category.}}
    \label{tab:kge_use}
\end{table*}

\DIFadd{The mHM routine exhibits the highest performance across most categories. This superiority is particularly evident in the outflow simulations, where it is only outperformed by LISFLOOD in the Navigation category. Because mHM is a routine fundamentally driven by water demand time series, it follows logically that it captures release dynamics more effectively than routines primarily governed by available storage. Despite this structural orientation, mHM still emerges as the top-performing routine for storage simulations in three out of the six reservoir use categories. Meanwhile, CaMa-Flood reproduces storage variations slightly better in Flood Control and Water Supply reservoirs. Finally, we note that the lower apparent performance in the Navigation and Recreation categories is likely an artifact of their very small sample sizes.
}
