...
def plot_reservoir(df, clim=None, nlim=None, flim=None, **kwargs):
    ...
    ax2 = ax.twinx()
    ax2.fill_between(df.index, df.filling, color='gray', alpha=.1, zorder=0, label=var)
    if clim is not None:
    ...
    fig.legend(ncol=2, bbox_to_anchor=[1, .8, .1, .1], frameon=False);
...
def plot_mapstacks(dct, agg='mean', **kwargs):
    ...
    # configure plot
    fig = plt.figure(figsize=kwargs.get('figsize', (5 * len(dct), 8)))
    gs = GridSpec(2, len(dct), figure=fig)
    ax_ts = fig.add_subplot(gs[1,:])
    cmaps = ['Blues', 'Greens', 'Reds', 'Purples', 'Oranges']
    colors = ['royalblue', 'forestgreen', 'firebrick', 'purple', 'orange']
